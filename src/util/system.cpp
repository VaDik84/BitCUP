...
const char * const BITCOIN_CONF_FILENAME = "bitcup.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcup-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitCUP
    // macOS: ~/Library/Application Support/BitCUP
    // Unix-like: ~/.bitcup
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitCUP";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitCUP";
#else
    // Unix-like
    return pathRet / ".bitcup";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty bitcup.conf if it does not exist
