...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 BCP irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on bitcupcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* BitCUP features */
    { "bitcup",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "bitcup",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "bitcup",               "gobject",                &gobject,                {} },
    { "bitcup",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
