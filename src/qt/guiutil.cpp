...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool fAllowURI)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitCUP address (e.g. %1)").arg(
...
void setupAppearance(QWidget* parent, OptionsModel* model)
{
    if (!QSettings().value("fAppearanceSetupDone", false).toBool()) {
        ...
        dlg.setWindowIcon(QIcon(":icons/bitcup"));
...
{
    // return if URI is not valid or is no bitcup: URI
    if(!uri.isValid() || uri.scheme() != QString("bitcup"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::BCP, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    QString ret = QString("bitcup:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BCP, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open bitcup.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitCUP Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitCUP Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BitCUP Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "BitCUP Core*.lnk"
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitcup.desktop";
    return GetAutostartDir() / strprintf("bitcup-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bitcup.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitCUP Core\n";
        else
            optionFile << strprintf("Name=BitCUP Core (%s)\n", chain);
...
void disableMacFocusRect(const QWidget* w)
{
    ...
    for (const auto& c : w->findChildren<QWidget*>()) {
        if (c->testAttribute(Qt::WA_MacShowFocusRect)) {
            c->setAttribute(Qt::WA_MacShowFocusRect, !bitcupThemeActive());
...
void updateMacFocusRects()
{
    ...
    while (it != setRectsDisabled.end()) {
        if (allWidgets.contains(*it)) {
            (*it)->setAttribute(Qt::WA_MacShowFocusRect, !bitcupThemeActive());
            ++it;
        } else {
