...
impl PrivateKey {
    ...
    #[cfg(feature = "bitcup_helpers")]
    pub fn generate_bitcup(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "bitcup_helpers")]
    pub fn generate_bitcup_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_bitcup(rng))
            .collect()
    }
