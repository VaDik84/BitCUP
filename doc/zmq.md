...
The BitCUP Core daemon can be configured to act as a trusted "border
router", implementing the bitcup wire protocol and relay, making
...
The ZeroMQ feature in BitCUP Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitcupd:
...
For instance:
    ...
    $ bitcupd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/bitcupd.tx.raw \
...
These options can also be provided in bitcup.conf.
...
From the perspective of bitcupd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitcupd directly. Furthermore, no information is
...
using. BitCUPd appends an up-counting sequence number to each
