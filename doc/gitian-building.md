...
*Setup instructions for a Gitian build of BitCUP Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitCUP
...
- [Building BitCUP Core](#building-bitcup-core)
...
Clone the git repositories for BitCUP Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/VaDik84
git clone https://github.com/bitcuppay/gitian.sigs.git
```
...
in the BitCUP Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building BitCUP Core
----------------

To build BitCUP Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the BitCUP Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcup/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/VaDik84
...
For example:
```bash
URL=https://github.com/crowning-/bitcup.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bitcup=${COMMIT} --url bitcup=${URL} ../bitcup/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcup=${COMMIT} --url bitcup=${URL} ../bitcup/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcup=${COMMIT} --url bitcup=${URL} ../bitcup/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitcup git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcup/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bitcup
SIGPATH=/some/root/path/bitcup-detached-sigs

./bin/gbuild --url bitcup=${BTCPATH},signature=${SIGPATH} ../bitcup/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcup-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcup-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcup-osx-build.assert
```
...
[bitcup/gitian.sigs](https://github.com/bitcuppay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
