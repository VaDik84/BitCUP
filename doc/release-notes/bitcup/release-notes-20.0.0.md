...
Currently, 10% of block rewards are set aside for the BitCUP DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking BitCUP in a
credit pool. This credit pool mirrors the available credit supply in BitCUP
Platform. When a certain amount of BitCUP is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on BitCUP
Platform. Masternode identities receive the fees and can convert them back to
BitCUP by withdrawing them from BitCUP Platform.
...
- `bestCLHeightDiff` (`uint32`) is equal to the diff in heights from the mined
  block height
- `creditPoolBalance` (`int64`) is equal to all the Bitosik locked in all previous
  blocks minus all the Bitosik unlocked in all previous blocks plus all the block
...
### Removal of reject network messages from BitCUP Core (BIP61)

This feature has been disabled by default since BitCUP Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since BitCUP Core version 20.0.0 there are extra changes:
...
- It is possible to run BitCUP Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `BCP/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from BitCUP Core (BIP61)_](#removal-of-reject-network-messages-from-bitcup-core-bip61) above for
...
### BitCUP-specific changes in existing RPCs:
...
- The `bitcup-cli -getinfo` command now displays the wallet name and balance for
...
python test framework and `bitcup-cli` tool both take advantage of this, so
for example:

```sh
bitcup-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
bitcup-cli -named createwallet mywallet load_on_startup=1
```
...
with BitCUP’s product needs, such as SegWit and RBF, are excluded from our
...
[DIP-0023]: https://github.com/bitcuppay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/bitcuppay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/VaDik84/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/VaDik84/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/VaDik84/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/VaDik84/compare/v19.3.0...bitcuppay:v20.0.0
