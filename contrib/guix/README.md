...
This directory contains the files necessary to perform bootstrappable BitCUP
Core builds.
...
To build BitCUP Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BitCUP Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitcup-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitcuppay/guix.sigs).
...
[guix-docker]: https://github.com/VaDik84/tree/master/contrib/guix/Dockerfile
